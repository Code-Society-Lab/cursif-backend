defmodule CursifWeb.PageChannel do
  use Phoenix.Channel

  alias Delta.Op

  def join("page:lobby", _message, socket) do
    {:ok, socket}
  end

  # need to load the page
  def join("page:" <> page_id, params, socket) do
    if authorized?(params) do
      pid = String.to_atom(page_id)
      Document.start_link(pid)

      socket = 
        socket
        |> assign(:page, Cursif.Pages.get_page!(page_id))
        |> assign(:pid, pid)
  ...
  def handle_in("update", %{"changes" => changes}, socket) do
    pid = socket.assigns.pid

    Document.update(pid, changes["ops"])
    broadcast_from(socket, "updated", %{"changes" => changes})

    {:noreply, socket}
  end

  defp authorized?(_payload),
    do: true
end
